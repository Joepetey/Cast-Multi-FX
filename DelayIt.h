...
public:
	...
	// --- cleaner code: function-ize the modes, also easier to learn
	//     You can combine these if you want
	enum {LEFT, RIGHT};
	float doNormalDelay(float xn, UINT channel);
	float doMultiTapDelay(float xn, UINT channel);

	// --- only works for stereo I/O
	void doPingPongDelay(float* pInputBuffer, float* pOutputBuffer, UINT delayType);
	...
	float m_fMTAttenuation_dB;
	float m_fPongRatio_pct;
	UINT m_uDelayType;
	enum{NORM,MTAP,PONG,MTPONG};

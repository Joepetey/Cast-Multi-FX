...
#include "ModDelayIt.h"

// --- there is no industry standard for these values
//     and I've set them so that they are not really perfect
//     First thing you should do is "voice" the plugin by 
//     testing/tweaking these values - use your ears
...
*/
bool __stdcall CModDelayIt::initialize()
{
	// Add your code here
...
processRackAFXMessage()
	...
	See the website www.willpirkle.com for more details
*/
void __stdcall CModDelayIt::processRackAFXMessage(UINT uMessage, PROCESS_INFO& processInfo)
{
...
bool __stdcall CModDelayIt::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	*/
	...
	// --- Flanger/Vibrato/Chorus: unipolar modulator
	...
	float feedback = m_uMode == FLANGER ? m_fFeedback_pct : 0.f;

	// --- get the delayed value
	float delayLeft = doNormalDelay(pInputBuffer[0], (m_fDepth_pct / 100.f)*modDelay_mSec, feedback, LEFT);

	// --- mix accordingly:
	if (m_uMode == FLANGER || m_uMode == CHORUS)
	{
		output = 0.5*pInputBuffer[0] + 0.5*delayLeft;
	}
	else if (m_uMode == VIBRATO)
	{
		output = delayLeft;
	}

	// --- write out
	pOutputBuffer[0] = output;
	...
	if (uNumInputChannels == 2 && uNumOutputChannels == 2)
	{
		...
		float modDelay_mSec = m_uLFOPhase ==  NORM ? calculateModDelayTime_mSec(lfoOut) : calculateModDelayTime_mSec(lfoQPOut);

		// --- setup feedback: what happens if you let the chorus have feedback?
		float feedback = m_uMode == FLANGER ? m_fFeedback_pct : 0.f;

		// --- get the delayed value
		float delayRight = doNormalDelay(pInputBuffer[0], (m_fDepth_pct / 100.f)*modDelay_mSec, feedback, RIGHT);

		// --- mix accordingly:
		if (m_uMode == FLANGER || m_uMode == CHORUS)
		{
			output = 0.5*pInputBuffer[0] + 0.5*delayRight;
		}
		else if (m_uMode == VIBRATO)
		{
			output = delayRight;
...
//
bool __stdcall CModDelayIt::userInterfaceChange(int nControlIndex)
{
	...
	switch(nControlIndex)
	{
		...
		case 41:
		{
			lfo.setOscWaveform(m_uLFOWaveForm); 
			break;
		}
		case 42:
		{
			if (m_uMode == FLANGER || m_uMode == VIBRATO)
			{
				minDelayMod_mSec = MIN_FLANGER_DELAY_MSEC;
				maxDelayMod_mSec = MAX_FLANGER_DELAY_MSEC;
			}
...
bool __stdcall CModDelayIt::checkUpdateGUI(int nControlIndex, float fValue, CLinkedList<GUI_PARAMETER>& guiParameters, bool bLoadingPreset)
{
...
bool __stdcall CModDelayIt::processAuxInputBus(audioProcessData* pAudioProcessData)
{
...
*/
bool __stdcall CModDelayIt::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD, float fACMix, float fBDMix)
{
	// add your code here
...
/* processAudioBuffer
	...
	******************************
	...
	m_HostProcessInfo.uTimeSigDenomintor = Host Time Signature Denominator (if supported by host)
*/
bool __stdcall CModDelayIt::processRackAFXAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
*/
bool __stdcall CModDelayIt::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
bool __stdcall CModDelayIt::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity)
{
	return true;
}

bool __stdcall CModDelayIt::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff)
{
	return true;
}

// uModValue = 0->127
bool __stdcall CModDelayIt::midiModWheel(UINT uChannel, UINT uModValue)
{
	return true;
}
...
bool __stdcall CModDelayIt::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue)
{
	return true;
}
...
*/
bool __stdcall CModDelayIt::midiClock()
{
...
bool __stdcall CModDelayIt::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
*/
void CModDelayIt::doVSTSampleAccurateParamUpdates()
{
...
void* __stdcall CModDelayIt::showGUI(void* pInfo)
{
...
bool __stdcall CModDelayIt::initUI()
{
	// ADDED BY RACKAFX -- DO NOT EDIT THIS CODE!!! -------------------------------------- //
...
// **--0xDEA7--**
	...
	ui3->m_pUserCookedUINTData = &m_uLFOWaveForm;
	ui3->cControlUnits = "";
	ui3->cVariableName = "m_uLFOWaveForm";
	ui3->cEnumeratedList = "TRI,SINE,UPSAW,DNSAW";
	...
	ui4->m_pUserCookedUINTData = &m_uMode;
	ui4->cControlUnits = "";
	ui4->cVariableName = "m_uMode";
	...
	ui5->m_pUserCookedUINTData = &m_uLFOPhase;
	ui5->cControlUnits = "";
	ui5->cVariableName = "m_uLFOPhase";

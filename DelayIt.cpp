...
*/
bool __stdcall CDelayIt::initialize()
{
	// Add your code here
...
processRackAFXMessage()
	...
	See the website www.willpirkle.com for more details
*/
void __stdcall CDelayIt::processRackAFXMessage(UINT uMessage, PROCESS_INFO& processInfo)
{
...
*/
bool __stdcall CDelayIt::prepareForPlay()
{
	// --- init the delay lines for 2 sec max delay - note this is sample-rate-dependent
...
//
float CDelayIt::doNormalDelay(float xn, UINT channel)
{
	...
	float delayInput = xn + (m_fFeedback_pct / 100.f)*delay;
...
/* --- Multi-tap delay
For this multi-tap algorithm, I am hardcoding the relationship of the taps:

tap1 = delay mSec, set on GUI
tap2 = 75% of tap1
tap3 = 50% of tap1

*/
float CDelayIt::doMultiTapDelay(float xn, UINT channel)
{
	// --- 1) read delay taps, decode channel
	float tap1 = channel == LEFT ? m_LeftDelay.readDelay_mSec(m_fDelay_mSec) : m_RightDelay.readDelay_mSec(m_fDelay_mSec);

	// --- 75%(time)
	float tap2 = channel == LEFT ? m_LeftDelay.readDelay_mSec(0.75f*m_fDelay_mSec) : m_RightDelay.readDelay_mSec(0.75f*m_fDelay_mSec);

	// --- 50%(time)
	float tap3 = channel == LEFT ? m_LeftDelay.readDelay_mSec(0.5f*m_fDelay_mSec) : m_RightDelay.readDelay_mSec(0.5f*m_fDelay_mSec);

	// --- form the total delay: note scaling MT attenuation value
	float atten = pow(10.f, m_fMTAttenuation_dB / 20.f);

	// --- mix of taps
	float delay = atten*tap1 + atten*tap2 + atten*tap3;
...
// --- caller must check to make sure we have stereo I/O! 
//     NOTE: there are several other ways you could handle this constraint
//           this is NOT the safest way since we depend on another function to check the setup
//           maybe you could improve this?
void CDelayIt::doPingPongDelay(float* pInputBuffer, float* pOutputBuffer, UINT delayType)
{
	float leftInput = pInputBuffer[0];
	float rightInput = pInputBuffer[1];

	// --- 1) read delays
	//
	// --- Left delay uses the GUI chosen delay time
	float delayL = m_LeftDelay.readDelay_mSec(m_fDelay_mSec);

	// --- right delay uses a fraction of this value, set with m_fPongRatio_pct
	float delayR = m_RightDelay.readDelay_mSec((m_fPongRatio_pct/100.f)*m_fDelay_mSec);

	// --- for multi-tap ping-pong
	if (delayType == MTPONG)
	{
		// --- MT attenuation
		float atten = pow(10.f, m_fMTAttenuation_dB / 20.f);
		...
		//
		// --- Left
		float leftTap2 = m_LeftDelay.readDelay_mSec(0.75f*m_fDelay_mSec);
		float leftTap3 = m_LeftDelay.readDelay_mSec(0.5f*m_fDelay_mSec);

		// --- scale and combine
		delayL = atten*delayL + atten*leftTap2 + atten*leftTap3;

		// --- Right (yes being very verbose, you could reuse variables...)
		float rightTap2 = m_RightDelay.readDelay_mSec(0.75f*(m_fPongRatio_pct / 100.f)*m_fDelay_mSec);
		float rightTap3 = m_RightDelay.readDelay_mSec(0.5f*(m_fPongRatio_pct / 100.f)*m_fDelay_mSec);

		// --- scale and combine
		delayR = atten*delayR + atten*rightTap2 + atten*rightTap3;
	}

	// --- 2) form delay  inputs, see block diagram 
	//        Left Delay Input = xn(right) + fb(right)
	float leftDelayInput = rightInput + (m_fFeedback_pct / 100.f)*delayR;

	//        Right Delay Input = xn(left) + fb(left)
	float rightDelayInput = leftInput + (m_fFeedback_pct / 100.f)*delayL;

	// --- 3) write input to delay lines
	m_LeftDelay.writeDelay(leftDelayInput);
	m_RightDelay.writeDelay(rightDelayInput);

	// --- 4) write outputs
	float mix = m_fDelayMix_pct / 100.f;

	// --- wet + dry
	pOutputBuffer[0] = mix*delayL + (1.f - mix)*leftInput;
	pOutputBuffer[1] = mix*delayR + (1.f - mix)*rightInput;
}
...
*/
bool __stdcall CDelayIt::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	// --- want to use the host BPM in some creative manner? 
	//     you can get it (and the time signature) from m_HostProcessInfo (see comment block above)
	//     I'm not using it in this plugin, just showing you how to get it
	//     see also: http://willpirkle.com/Downloads/DAW%20Host%20Information.pdf
	float tempoBPM = m_HostProcessInfo.dBPM;

	// get input
	float xnLeft = pInputBuffer[0];

	// --- ping-pong modes require stereo files to work properly, so check this
	bool pongWithMonoFile = false;
	if (m_uDelayType == PONG || m_uDelayType == MTPONG)
	{
		// --- mono-in/out
		if (uNumInputChannels == 1 && uNumOutputChannels == 1)
			pongWithMonoFile = true;
	}

	// --- decode and handle left channel AND ping-pong (both channels)
	if (m_uDelayType == NORM || pongWithMonoFile)
		pOutputBuffer[0] = doNormalDelay(xnLeft, LEFT);
	else if (m_uDelayType == MTAP)
		pOutputBuffer[0] = doMultiTapDelay(xnLeft, LEFT);
	else if (m_uDelayType == PONG || m_uDelayType == MTPONG)
		doPingPongDelay(pInputBuffer, pOutputBuffer, m_uDelayType);

	// Mono-In, Stereo-Out (AUX Effect)
	if (uNumInputChannels == 1 && uNumOutputChannels == 2)
		pOutputBuffer[1] = pOutputBuffer[0]; // <--- copy left to right

	// Stereo-In, Stereo-Out (INSERT Effect)
	if (uNumInputChannels == 2 && uNumOutputChannels == 2)
	{
		// get input
		float xnRight = pInputBuffer[1];

		if (m_uDelayType == NORM || pongWithMonoFile)
			pOutputBuffer[1] = doNormalDelay(xnRight, RIGHT);
		else if (m_uDelayType == MTAP)
			pOutputBuffer[1] = doMultiTapDelay(xnLeft, RIGHT);
		// ping-pongs are taken care of above 
	}
...
1				m_fFeedback_pct                   1
2				m_fDelayMix_pct                   2
3				m_fMTAttenuation_dB               3
4				m_fPongRatio_pct                  4
5				m_uDelayType                      41
...
//
bool __stdcall CDelayIt::userInterfaceChange(int nControlIndex)
{
...
bool __stdcall CDelayIt::checkUpdateGUI(int nControlIndex, float fValue, CLinkedList<GUI_PARAMETER>& guiParameters, bool bLoadingPreset)
{
...
bool __stdcall CDelayIt::processAuxInputBus(audioProcessData* pAudioProcessData)
{
...
*/
bool __stdcall CDelayIt::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD, float fACMix, float fBDMix)
{
	// add your code here
...
/* processAudioBuffer
	...
	******************************
	...
	m_HostProcessInfo.uTimeSigDenomintor = Host Time Signature Denominator (if supported by host)
*/
bool __stdcall CDelayIt::processRackAFXAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
*/
bool __stdcall CDelayIt::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
bool __stdcall CDelayIt::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity)
{
	return true;
}

bool __stdcall CDelayIt::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff)
{
	return true;
}

// uModValue = 0->127
bool __stdcall CDelayIt::midiModWheel(UINT uChannel, UINT uModValue)
{
	return true;
}
...
bool __stdcall CDelayIt::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue)
{
	return true;
}
...
*/
bool __stdcall CDelayIt::midiClock()
{
...
bool __stdcall CDelayIt::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
*/
void CDelayIt::doVSTSampleAccurateParamUpdates()
{
...
void* __stdcall CDelayIt::showGUI(void* pInfo)
{
...
bool __stdcall CDelayIt::initUI()
{
	// ADDED BY RACKAFX -- DO NOT EDIT THIS CODE!!! -------------------------------------- //
...
// **--0xDEA7--**
	...
	ui0->nGUIColumn = -1;
	ui0->bEnableParamSmoothing = true;
	ui0->fSmoothingTimeInMs = 2000.00;
	...
	ui1->nGUIColumn = -1;
	ui1->bEnableParamSmoothing = true;
	...
	ui2->m_pUserCookedFloatData = &m_fDelayMix_pct;
	ui2->m_pUserCookedDoubleData = NULL;
	ui2->m_pUserCookedUINTData = NULL;
	ui2->cControlUnits = "%";
	ui2->cVariableName = "m_fDelayMix_pct";
	...
	ui2->nGUIColumn = -1;
	ui2->bEnableParamSmoothing = true;
	...
	ui4->fUserDisplayDataHiLimit = 99.000000;
	...
	ui4->m_pUserCookedFloatData = &m_fPongRatio_pct;
	ui4->m_pUserCookedDoubleData = NULL;
	ui4->m_pUserCookedUINTData = NULL;
	ui4->cControlUnits = "%";
	ui4->cVariableName = "m_fPongRatio_pct";
	...
	ui5->fUserDisplayDataHiLimit = 3.000000;
	...
	ui5->m_pUserCookedUINTData = &m_uDelayType;
	ui5->cControlUnits = "";
	ui5->cVariableName = "m_uDelayType";
	ui5->cEnumeratedList = "NORM,MTAP,PONG,MTPONG";
